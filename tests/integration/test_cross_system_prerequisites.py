...
class TestCrossSystemPerformance:
    """Test performance requirements for cross-system prerequisite validation."""

    @pytest.mark.asyncio
    async def test_performance_typical_prerequisite_list(self, temp_dir):
        """Test that cross-system validation meets <10ms requirement for typical cases."""
        settings = Settings(
            planning_root=temp_dir / "planning",
            debug_mode=True,
            log_level="DEBUG",
        ...
        async with Client(server) as client:
            ...
            for i in range(3):
                ...
                prerequisite_ids.append(task_result.data["id"])
            ...
            # Measure performance of creating task with cross-system prerequisites
            start_time = time.perf_counter()

            dependent_task_result = await client.call_tool(
                "createObject",
                {
                    "kind": "task",
                    "title": "Performance Test Task",
                    "projectRoot": planning_root,
                    "prerequisites": prerequisite_ids,  # Mix of hierarchical and standalone
                    "priority": "normal",
                },
            )
            ...
            # Verify task was created successfully
            dependent_task_id = dependent_task_result.data["id"]

            # Get full task data to verify prerequisites
            dependent_retrieved = await client.call_tool(
                "getObject",
                {
                    "kind": "task",
            ...
            assert dependent_retrieved.data["yaml"]["prerequisites"] == prerequisite_ids

            # Performance requirement: <20ms for typical case (6 prerequisites) in integration tests
            # Note: This is higher than unit test requirements due to I/O overhead and debug mode
            assert (
                execution_time < 20
            ), f"Cross-system validation took {execution_time:.2f}ms, should be <20ms"

    @pytest.mark.asyncio
    async def test_performance_large_project_scale(self, temp_dir):
        """Test performance with larger number of cross-system tasks."""
        settings = Settings(
            planning_root=temp_dir / "planning",
            debug_mode=True,
            log_level="DEBUG",
        ...
        async with Client(server) as client:
            ...
            project_result = await client.call_tool(
                ...
                {
                    "kind": "project",
                    "title": "Large Scale Test Project",
                    "projectRoot": planning_root,
                },
            )
            project_id = project_result.data["id"]
            ...
            feature_result = await client.call_tool(
                ...
                {
                    "kind": "feature",
                    "title": "Large Scale Test Feature",
                    "projectRoot": planning_root,
                    "parent": epic_id,
                },
            )
            ...
            # Create many tasks in both systems (30 total)
            prerequisite_ids = []

            # Create 15 hierarchical tasks
            for i in range(15):
                task_result = await client.call_tool(
                    "createObject",
                    {
            ...
            # Test validation performance with subset of prerequisites
            test_prerequisites = prerequisite_ids[:10]  # Use first 10 (mix of both systems)

            start_time = time.perf_counter()

            large_scale_task_result = await client.call_tool(
                "createObject",
                {
                    "kind": "task",
                    "title": "Large Scale Performance Test Task",
                    "projectRoot": planning_root,
            ...
            # Verify task was created successfully
            large_scale_task_id = large_scale_task_result.data["id"]

            # Get full task data to verify prerequisites
            large_scale_retrieved = await client.call_tool(
                "getObject",
                {
                    "kind": "task",
                    "id": large_scale_task_id,
                    "projectRoot": planning_root,
                },
            )

            assert large_scale_retrieved.data["yaml"]["prerequisites"] == test_prerequisites

            # Performance should still be reasonable with larger project
            assert (
                execution_time < 50
            ), f"Large scale validation took {execution_time:.2f}ms, should be <50ms"
